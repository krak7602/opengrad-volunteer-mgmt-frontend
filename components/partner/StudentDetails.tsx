...
export function StudentDetails({ studId }: { studId: string }) {
  ...
  const volunteerForStudent = useFetch<vol>(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/volById/${data?.volId}`,
    {
      headers: {
        authorization: `Bearer ${session.data?.user.auth_token}`,
      },
            ...
            <div>
              <div className="grid gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  <div className="grid grid-cols-3 items-center gap-4">
                    <Label htmlFor="email">Volunteer</Label>
                    <div className="col-span-2 h-6">
                      {volunteerForStudent?.data?.user_id?.name}
                    </div>
                  </div>
                </div>
              </div>
