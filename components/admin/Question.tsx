        ...
        <div className="rounded-sm bg-gray-100 border p-2">
            <div className=" flex flex-col">
                ...
                <div className="flex flex-row justify-between items-center pb-1">
                    ...
                    {type === "desc" && <div className=' font-light text-xs'>
                        Descriptive
                    </div>}
                ...
                <Textarea className="min-h-[100px] min-w-max" onChange={(e) => { handleDescChange(e, index) }} />
                {type === "mcq" && optionCount >= 1 && <div>
                    <div>A</div>
                    <Textarea className="min-h-1 min-w-max" placeholder="Option description" onChange={(e) => { handleOptionsChange(e, index, 0) }} />
                </div>}
                {type === "mcq" && optionCount >= 2 && <div>
                    <div>B</div>
                    <Textarea className="min-h-1 min-w-max" placeholder="Option description" onChange={(e) => { handleOptionsChange(e, index, 1) }}/>
                </div>}
                {type === "mcq" && optionCount >= 3 && <div>
                    <div>C</div>
                    <Textarea className="min-h-1 min-w-max" placeholder="Option description" onChange={(e) => { handleOptionsChange(e, index, 2) }}/>
                </div>}
                {type === "mcq" && optionCount >= 4 && <div>
                    <div>D</div>
                    <Textarea className="min-h-1 min-w-max" placeholder="Option description" onChange={(e) => { handleOptionsChange(e, index, 3) }}/>
                </div>}
                {type === "mcq" && <Button onClick={() => handleAddOption(index)} type="button" className=" mt-2 flex items-center justify-center gap-2" variant="outline">
